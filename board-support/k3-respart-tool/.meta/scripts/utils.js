...
const tifsResRem = _.keyBy(system.getScript("/.meta/data/tifsResRem.json"), (r) => r);
const socName = devData[deviceSelected].shortName;

const resources = _.keyBy(system.getScript("/.meta/data/" + socName + "/Resources.json"), (r) => r.utype);
var FirewallDevices = _.keyBy(system.getScript("/.meta/data/" + socName + "/Firewall.json"), (d) => d.name);
const qos = _.keyBy(system.getScript("/.meta/data/" + socName + "/Qos.json"), (r) => r.endpointName);
const hosts = _.keyBy(system.getScript("/.meta/data/" + socName + "/Hosts.json"), (r) => r.hostName);
const bwlimiter = _.keyBy(system.getScript("/.meta/data/" + socName + "/BWlimiters.json"), (d) => d.deviceName);
...
function getQosValueUboot(inst) {
	var value = "";

	inst.qos?value += inst.qos + ", ":value += "0, ";
	inst.orderId?value += inst.orderId + ", ":value += "0, ";
	inst.asel?value += inst.asel + ", ":value += "0, ";
	inst.epriority?value += inst.epriority + ", ":value += "0, ";
	inst.virtId?value += inst.virtId + ", ":value += "0, ";
	inst.atype?value += inst.atype:value += "0";
