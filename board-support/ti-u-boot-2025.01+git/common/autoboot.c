 ...
 * Return: 0 if autoboot should continue, 1 if it should stop
 */
static int passwd_abort_key(uint64_t etime, int bootdelay)
{
	int abort = 0;
	struct {
		char *str;
	...
	/*
	 * In AUTOBOOT_KEYED mode, U-Boot historically prints the prompt once.
	 * Add a real countdown refresh (similar UX to abortboot_single_key()).
	 *
	 * IMPORTANT: CONFIG_AUTOBOOT_PROMPT should NOT contain '\n', otherwise
	 * the countdown refresh would spam multiple lines.
	 */
	u64 next_tick = get_ticks() + get_tbclk();
	int seconds_left = bootdelay;

	printf("\r");
	printf(CONFIG_AUTOBOOT_PROMPT, seconds_left);
#endif
...
#  endif
	...
	do {
		...
		if (!abort && get_ticks() >= next_tick && seconds_left > 0) {
			seconds_left--;
			next_tick += get_tbclk();
			printf("\r");
			printf(CONFIG_AUTOBOOT_PROMPT, seconds_left);
...
static int abortboot_key_sequence(int bootdelay)
{
	...
	if (IS_ENABLED(CONFIG_AUTOBOOT_ENCRYPTION)) {
		...
		abort = passwd_abort_key(etime, bootdelay);
	}
	if (!abort)
